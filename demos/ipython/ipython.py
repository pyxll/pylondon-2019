...
#@xl_menu("Open Notebook", menu="IPython")
def ipython_notebook(*args):
    """
    ...
    try:
        ...
        # start a subprocess to run the Qt console
        # run jupyter in it's own process
        _launch_jupyter(app.connection_file)
    except:
        if win32api:
            win32api.MessageBox(None, "Error starting Jupyter notebook")
        _log.error("Error starting Jupyter notebook", exc_info=True)
...
def ipython_start_kernel(*args):
    ...
    try:
        ...
        win32api.MessageBox(None, "IPython kernel started: %s" % app.connection_file)
    except:
        if win32api:
            win32api.MessageBox(None, "Error starting IPython Qt console")
        _log.error("Error starting IPython Qt console", exc_info=True)
...
def _launch_jupyter(connection_file):
    from subprocess import Popen

    # Find juypter-notebook.exe in the Scripts path local to python.exe
    exe = None
    if sys.executable and os.path.basename(sys.executable) in ("python.exe", "pythonw.exe"):
        path = os.path.join(os.path.dirname(sys.executable), "Scripts")
        exe = os.path.join(path, "jupyter-notebook.exe")

    # If it wasn't found look for it on the system path
    if exe is None or not os.path.exists(exe):
        exe = _which("jupyter-notebook.exe")
    ...
    # Cd to ../notebooks before starting
    os.chdir(os.path.join(os.path.dirname(__file__), "..", "notebooks"))

    # run jupyter in it's own process
    cmd = [exe, "--NotebookApp.kernel_manager_class=extipy.ExternalIPythonKernelManager"]
